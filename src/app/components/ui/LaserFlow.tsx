...
/**
 * Volumetric laser beam WebGL background from React Bits.
 */
export default function LaserFlow({
  ...
  useEffect(() => {
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < 50) {
        next = Math.max(0.6, Math.min(base, currentDprRef.current * 0.85));
      } else if (avgFps > 58 && currentDprRef.current < base) {
        next = Math.max(0.6, Math.min(base, currentDprRef.current * 1.1));
      }
  ...
  return (
    <div
      ref={mountRef}
      className={`laser-flow-container ${className ?? ""}`.trim()}
      style={style}
      aria-hidden="true"
    />
  );
